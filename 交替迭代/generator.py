...
def volgen(
    ...
    while True:
        ...
        imgs = [utils.load_volfile(vol_names[i], **load_params) for i in indices] #vol_names列表不变的情况下，这里获取的index一定是按顺序不变的
        # imgs[0] = imgs[0][48:-48, 31:-33, 3:-29]
        # print(imgs[0].shape)
...
def scan_to_scan(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    ...
    while True:
        index1, scan1 = next(gen)
        index2, scan2 = next(gen)
        scan1 = scan1[0]
        scan2 = scan2[0]
        ...
        invols = [scan1, scan2]
        outvols = [scan2, scan1] if bidir else [scan2] # 目前看来都是scan2
        if not no_warp:
            outvols.append(zeros)

        yield [index1, index2], (invols, outvols) # 都包含2组数：(1, 144, 192, 160, 1). outvols中，后一组数为zeros.
